...
from ..base_inferences.perframe_det_batch_inference import do_perframe_det_batch_inference, do_ek100_perframe_det_batch_inference, do_ek100_perframe_det_batch_inference_rev, do_perframe_det_batch_inference_rev
...
def do_lstr_batch_inference(cfg,
                            ...
                            logger):
    if cfg.MODEL.LSTR.INFERENCE_MODE == 'stream':
        do_lstr_stream_inference_rev(cfg,
                                 model, rev_model,
                                 device,
                                 logger)
    else:
        if cfg.DATA.DATA_NAME == 'EK100':
            do_ek100_perframe_det_batch_inference_rev(cfg,
                                            model, rev_model,
                                            device,
                                            logger)
        else:
            do_perframe_det_batch_inference_rev(cfg,
                                            model, rev_model,
                                            device,
                                            logger)
...
def do_lstr_stream_inference_rev(cfg, model, device, logger):
    # Setup model to test mode
    model.eval()
