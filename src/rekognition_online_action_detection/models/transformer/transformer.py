...
from .multihead_attention import DisentangledAttentionStream as DisnentangledAttention
    ...
    def forward(self, tgt, memory, tgt_mask=None,
        ...
        for mod in self.layers:
            output = mod(output, memory, tgt_mask=tgt_mask,
                         memory_mask=memory_mask,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask, knn=knn)
...
class Disentangledtransformer(nn.Module):
    ...
    def forward(self, slots, tgt, memory, tgt_mask=None,
                memory_mask=None, tgt_key_padding_mask=None,
                memory_key_padding_mask=None, knn=False):
        output = tgt

        for mod in self.layers:
            output = mod(slots, output, memory, tgt_mask=tgt_mask,
                         memory_mask=memory_mask,
                         tgt_key_padding_mask=tgt_key_padding_mask,
                         memory_key_padding_mask=memory_key_padding_mask, knn=knn)
...
class DisentangledLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation='relu'):
        super(DisentangledLayer, self).__init__()

        self.self_attn = DisnentangledAttention(d_model, nhead, dropout=dropout)
        self.multihead_attn = DisnentangledAttention(d_model, nhead, dropout=dropout)
        ...
        self.linear_tgt_k = nn.Linear(dim_feedforward, d_model)
        self.linear_tgt_v = nn.Linear(dim_feedforward, d_model)
        self.linear_memory_k = nn.Linear(dim_feedforward, d_model)
        self.linear_memory_v = nn.Linear(dim_feedforward, d_model)
    ...
    def __setstate__(self, state):
        if 'activation' not in state:
            state['activation'] = F.relu
        super(DisentangledLayer, self).__setstate__(state)
    ...
    def forward(self, slots, tgt, memory, tgt_mask=None, memory_mask=None,
                tgt_key_padding_mask=None, memory_key_padding_mask=None, knn=False):
        
        
        tgt2 = self.self_attn(slots, tgt, tgt, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask, knn=knn)[0]
        
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        tgt2 = self.multihead_attn(slots, memory, memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask, knn=knn)[0]
        tgt = tgt + self.dropout2(tgt2)
        tgt = self.norm2(tgt)
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt))))
