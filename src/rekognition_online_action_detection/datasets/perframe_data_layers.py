...
class LSTRDataLayer(data.Dataset):
    ...
    def __init__(self, cfg, phase='train'):
        ...
        self.audio_feature = cfg.INPUT.audio_FEATURE
    ...
    def __getitem__(self, index):
        ...
        audio_inputs = np.load(
            osp.join(self.data_root, self.audio_feature, session + '.npy'), mmap_mode='r')
        ...
        if self.future_num_samples > 0:
            ...
            work_audio_inputs = audio_inputs[work_indices]
        else:
            target = target[work_start: work_end][::self.work_memory_sample_rate]
            work_indices = np.arange(work_start, work_end).clip(0)
            work_indices = work_indices[::self.work_memory_sample_rate]
        ...
        if self.long_memory_num_samples > 0:
            ...
            long_audio_inputs = audio_inputs[long_indices]
        ...
        if long_visual_inputs is not None and long_motion_inputs is not None and long_audio_inputs is not None:
            ...
            fusion_audio_inputs = np.concatenate((long_audio_inputs, work_audio_inputs))
        else:
            fusion_visual_inputs = work_visual_inputs
            fusion_motion_inputs = work_motion_inputs
            fusion_audio_inputs = work_audio_inputs
        ...
        fusion_audio_inputs = torch.as_tensor(fusion_audio_inputs.astype(np.float32))
        ...
        if memory_key_padding_mask is not None:
            ...
            if future_target is not None:
                ...
                return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, memory_key_padding_mask, (target, future_target)
            return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, memory_key_padding_mask, target
        else:
            if future_target is not None:
                future_target = torch.as_tensor(future_target.astype(np.float32))
                return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, (target, future_target)
            return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, target
...
class LSTRDataLayer_(data.Dataset):
    ...
    def _init_dataset(self):
        ...
        for session in self.sessions:
            ...
            self.audio[session] = np.load(osp.join(self.data_root, self.audio_feature, session + '.npy'))
    ...
    def __getitem__(self, index):
        ...
        audio_inputs = self.audio[session]
        ...
        if self.training and self.clip_mixup_rate > 0:
            ...
            for segment in segments_to_mix.iterrows():
                ...
                if len(valid_segments) == 0:
                    ...
                    new_audio_inputs = self.audio[new_vid]
                    ...
                    work_audio_inputs[sel_indices] = self.plus_ratio * new_audio_inputs[new_start_tick + shift:new_start_tick + shift + len(sel_indices)] + (1 - self.plus_ratio) * \
                                                      work_audio_inputs[sel_indices]
            ...
            if self.training and self.clip_mixup_rate > 0:
                ...
                for old_segment in segments_to_mixup.iterrows():
                    ...
                    if len(valid_segments) == 0:
                        ...
                        long_audio_inputs[sel_indices] = new_audio_inputs[new_start_tick + shift:new_start_tick + shift + len(sel_indices)]
        ...
        if memory_key_padding_mask is not None:
            ...
            if future_target is not None:
                ...
                return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, memory_key_padding_mask, (target, noun_target, verb_target, future_target, future_noun_target, future_verb_target)
            return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, memory_key_padding_mask, (target, noun_target, verb_target)
        else:
            if future_target is not None:
                future_target = torch.as_tensor(future_target.astype(np.float32))
                verb_target = torch.as_tensor(verb_target.astype(np.float32))
                ...
                future_noun_target = torch.as_tensor(future_noun_target.astype(np.float32))
                future_verb_target = torch.as_tensor(future_verb_target.astype(np.float32))
                return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, (target, noun_target, verb_target, future_target, future_noun_target, future_verb_target)
            return fusion_visual_inputs, fusion_motion_inputs, fusion_audio_inputs, (target, noun_target, verb_target)
