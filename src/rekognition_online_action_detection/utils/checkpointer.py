...
class Checkpointer_rev(object):

    def __init__(self, cfg, phase):

        # Load pretrained checkpoint
        self.checkpoint = self._load_checkpoint(cfg.MODEL.CHECKPOINT_rev)
        if self.checkpoint is not None and phase == 'train':
            cfg.SOLVER.START_EPOCH += self.checkpoint.get('epoch', 0)
        elif self.checkpoint is None and phase != 'train':
            raise RuntimeError('Cannot find checkpoint {}'.format(cfg.MODEL.CHECKPOINT_rev))
    ...
    def save(self, epoch, model, optimizer):
        torch.save({
            ...
            'model_state_dict': model.module.state_dict() if torch.cuda.device_count() > 1 else model.state_dict(),
            'optimizer_state_dict': optimizer.state_dict(),
        }, osp.join(self.output_dir, 'epoch-{}_rev.pth'.format(epoch)))
...
def setup_checkpointer_rev(cfg, phase):
    return Checkpointer_rev(cfg, phase)