...
def do_perframe_det_batch_inference_rev(cfg, model,rev_model, device, logger):
    # Setup model to test mode
    model.eval()
...
def do_ek100_perframe_det_batch_inference_rev(cfg, model,rev_model, device, logger):
    # Setup model to test mode
    model.eval()
    ...
    for batch_idx, data in enumerate(pbar, start=1):
        if cfg.MODEL.LSTR.FUTURE_NUM_SAMPLES > 0:
            total_target, _, = data[-4]
            target, noun_target, verb_target = total_target
            a=[x.to(device) for x in data[:-4]]
            det_scores, _, noun_score, _, verb_score, _, det_feat,mem  = model(*[x.to(device) for x in data[:-4]])
            score = det_scores[-1]

            initial_pred = det_feat.detach() 
            initial_pred.requires_grad = True
            
            rev_score, rev_noun_score, rev_verb_score = rev_model(initial_pred)
            
            aloss = F.l1_loss(rev_score, score)
            nloss = F.l1_loss(rev_noun_score, noun_score)
            vloss = F.l1_loss(rev_verb_score, verb_score)
            aerr = aloss 
            verr = vloss 
            nerr = nloss
            model.zero_grad()
            rev_model.zero_grad()
            aerr.backward()
            data_grad = initial_pred.grad.data
            
            #1
            a_perturbed_data = fgsm_attack(initial_pred, eps, data_grad) #0.1 적용 시 0.586745 하락 (verb, noun 전부 하락)
            action_score, _, _= model.final_cls(a_perturbed_data)
            ...
            #2
            n_perturbed_data = fgsm_attack(initial_pred, eps, data_grad) #0.1 적용 시 0.586745 하락 (verb, noun 전부 하락)
            _, noun_score, _ = model.final_cls(n_perturbed_data)
            ...
            # 3
            v_perturbed_data = fgsm_attack(initial_pred, eps, data_grad) #0.1 적용 시 0.586745 하락 (verb, noun 전부 하락)
            _, _, verb_score = model.final_cls(v_perturbed_data)

            score = action_score.cpu().detach().numpy()
            score_verb = verb_score.cpu().detach().numpy()
            score_noun = noun_score.cpu().detach().numpy()
        else:
            score = model(*[x.to(device) for x in data[:-4]])
            target, noun_target, verb_target = data[-4]
            score = score.cpu().numpy()
    ...
    scores = pd.DataFrame(all_accuracies * 100, columns=cc,
                          index=pd.MultiIndex.from_tuples(indices))
    scores.to_csv('ek100_scores.csv')
    
    logger.info(scores)
    logger.info(f'\nEpsilon: {eps:0.3f}\t Only {loss_name}')
    tta_verb = tta(verb_scores, verb_labels)
    tta_noun = tta(noun_scores, noun_labels)
    tta_action = tta(action_scores, action_labels)
    logger.info(f'\nMean TtA(5): VERB {tta_verb:0.2f} '
