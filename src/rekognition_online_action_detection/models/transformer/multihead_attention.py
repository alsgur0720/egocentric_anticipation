...
class DotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None, knn=False):
        ...
        # print(attn_output_weights.shape, "attn_output_weights")
        
        # print(torch.max(attn_output_weights), "attn_output_weights")
        # print(v.shape, "v.shape")
        # import matplotlib.pyplot as plt
        # tensor1 = attn_output_weights
        # tensor1_np = tensor1.cpu().numpy()
        # tensor_np1_copy = tensor1_np[64,:,:].copy()
        # plt.figure(figsize=(16, 16))
        # plt.imshow(tensor_np1_copy.T)
        # plt.savefig("disentangled_attn_test.jpg")
        # plt.close()

        # tensor2 = v
        # tensor2_np = tensor2.cpu().numpy()
        # tensor_np2_copy = tensor2_np[64,:,:].copy()
        # plt.figure(figsize=(16, 512))
        # plt.imshow(tensor_np2_copy.T)
        # plt.savefig("disentangled_v.jpg")
        # plt.close()
        ...
        # tensor_np3_copy = tensor3_np[64,:,:].copy()
        # plt.figure(figsize=(16, 512))
        # plt.imshow(tensor_np3_copy.T)
        # plt.savefig("disentangled_attn_output.jpg")
        # plt.close()
        
        return attn_output

class DotProductAttention_disentangle(nn.Module):

    def __init__(self, dim, dropout=0.0):
        super(DotProductAttention_disentangle, self).__init__()
    ...
    def forward(self, q, k, v, attn_mask=None, knn=False):
        ...
        attn_output = self.gru(
            attn_output.reshape(-1, 512),
            q.reshape(-1, 512)
        )
        attn_output = attn_output.reshape(-1, N2, 512)
...
class DotProductAttentionStream_disentangle(DotProductAttention_disentangle):

    def __init__(self, dim, dropout=0.0):
        super(DotProductAttentionStream_disentangle, self).__init__(dim, dropout)
...
class DisnentangledAttention(nn.Module):

    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        super(DisnentangledAttention, self).__init__()
        ...
        self.dotproductattention = DotProductAttention_disentangle(dropout, embed_dim)
...
class DisentangledAttentionStream(DisnentangledAttention):

    def __init__(self, embed_dim, num_heads, dropout=0.0, bias=True, kdim=None, vdim=None):
        super(DisentangledAttentionStream, self).__init__(embed_dim, num_heads, dropout, bias, kdim, vdim)

        self.dotproductattention = DotProductAttentionStream_disentangle(embed_dim, dropout)
