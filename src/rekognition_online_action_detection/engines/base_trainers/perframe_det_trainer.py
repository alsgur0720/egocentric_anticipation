...
def do_perframe_det_train_rev(cfg,
                          data_loaders,
                          model, rev_model,
                          criterion,
                          optimizer,
...
def do_ek100_perframe_det_train_rev(cfg,
                          data_loaders,
                          model, rev_model,
                          criterion,
                          optimizer,
    ...
    for epoch in range(cfg.SOLVER.START_EPOCH, cfg.SOLVER.START_EPOCH + cfg.SOLVER.NUM_EPOCHS):
        # Reset
        rev_losses = {phase: 0.0 for phase in cfg.SOLVER.PHASES}
        fut_losses = {phase: 0.0 for phase in cfg.SOLVER.PHASES}
        det_pred_scores = []
        det_gt_targets = []
        ...
        for phase in cfg.SOLVER.PHASES:
            ...
            with torch.set_grad_enabled(training):
                ...
                for batch_idx, data in enumerate(pbar, start=1):
                    ...
                    det_scores, fut_scores, noun_score, fut_noun_score, verb_score, fut_verb_score, det_feat = model(*[x.to(device) for x in data[:-1]])

                    rev_score, rev_noun_score, rev_verb_score = rev_model(det_feat) # 12개 인데1~12
                    rev_score = rev_score[:,1:] #~ 2~12초까지
                    rev_score = rev_score.reshape(-1, cfg.DATA.NUM_CLASSES)
                    det_target = det_target[:,:11,:] # 1~11초까지

                    rev_noun_score = rev_noun_score[:,1:] #~ 2~12초까지
                    noun_target = noun_target[:,:11,:] # 1~11초까지

                    rev_verb_score = rev_verb_score[:,1:] #~ 2~12초까지
                    verb_target = verb_target[:,:11,:] # 1~11초까지
                    ...
                    rev_loss = criterion['MCE_EQL'](rev_score, det_target)
                    rev_verb_score, rev_noun_score = rev_verb_score.reshape(-1, rev_verb_score.shape[-1]), rev_noun_score.reshape(-1, rev_noun_score.shape[-1])
                    verb_loss = criterion['MCE'](rev_verb_score, verb_target)
                    noun_loss = criterion['MCE'](rev_noun_score, noun_target)
                    rev_loss += (verb_loss + noun_loss)
                    
                    rev_losses[phase] += rev_loss.item() * batch_size

                    # Output log for current batch
                    pbar.set_postfix({
                        'lr': '{:.7f}'.format(scheduler.get_last_lr()[0]),
                        'rev_loss': '{:.5f}'.format(rev_loss.item()),
                    })
                    ...
                    if training:
                        ...
                        # Prepare for evaluation
                        rev_score = rev_score.softmax(dim=1).cpu().tolist()
                        det_target = det_target.cpu().tolist()
                        det_pred_scores.extend(rev_score)
                        det_gt_targets.extend(det_target)
        end = time.time()
        ...
        log = []
        log.append('Epoch {:2}'.format(epoch))
        log.append('train rev_loss: {:.5f}'.format(
            rev_losses['train'] / len(data_loaders['train'].dataset),
        ))
        if 'test' in cfg.SOLVER.PHASES:
            # Compute result
            gt, pred = np.array(det_gt_targets), np.array(det_pred_scores)[:, 1:]
            ...
            valid_index = list(np.where(action_labels != 0))[0]
            det_result = topk_recall_multiple_timesteps(action_pred[valid_index, ...], action_labels[valid_index], k=5)[0]
            log.append('test rev_loss: {:.5f}, rev_Recall: {:.5f}'.format(
                rev_losses['test'] / len(data_loaders['test'].dataset),
                float(det_result) * 100,
            ))
        log.append('running time: {:.2f} sec'.format(
            end - start,
        ...
        # Save checkpoint for model and optimizer
        checkpointer.save(epoch, rev_model, optimizer)
        if not training:
            ema.restore()
